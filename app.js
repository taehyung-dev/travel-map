(function () {
  ...
  function resyncOneReveal(id) {
    ...
    if (!bbox.width || !bbox.height) {
      // Zero here isn't just a mid-zoom read racing ahead of Leaflet's own
      // path redraw - Leaflet also clips each path's points to a padded
      // box around the viewport for performance, so a region panned far
      // enough off-screen legitimately has no geometry until it's back in
      // range (see the 'moveend' listener below). Either way, leaving the
      // sticker at its last good position and retrying next time this
      // fires is much safer than deleting it outright, since a delete here
      // also erases it from this user's synced cloud record. Previously
      // this called clearOneReveal(id), which is exactly the repeated
      // "reveals vanish while zooming" bug users hit.
      return;
    }
  ...
  // Zooming always re-projects and rewrites every path's `d`. Panning
  // usually doesn't - it's just a shared CSS transform, so open reveals tag
  // along for free - EXCEPT Leaflet also clips each path's points to a
  // padded box around the current viewport for performance (see
  // Path._clipPoints/clipPolygon in Leaflet's source): pan a revealed
  // region far enough off-screen and Leaflet collapses its `d` to nothing,
  // which is exactly why resyncOneReveal's zero-bbox guard skips it rather
  // than deleting it. Panning back past that same boundary makes Leaflet
  // restore the real `d` again - if that pan-back was pure pan (no zoom in
  // between), only listening for 'zoomend' would never re-run the resync,
  // leaving the clip shape/image stuck at whatever stale `d`/bbox they had
  // from before the region went off-screen (from a possibly different zoom
  // level entirely) - a huge, wrongly clipped/positioned sticker. Listening
  // on 'moveend' instead covers both cases, since a zoom fires 'moveend'
  // too. A rAF after it avoids a race where this runs before Leaflet has
  // actually finished writing the new `d` attributes for that same event.
  map.on("moveend", function () {
